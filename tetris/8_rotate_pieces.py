...
def draw_moving_piece(screen, piece):
    # TODO Update the line below to take into account the piece rotation
...
def update_game_matrix(matrix, piece):
    for row in range(5):
        for col in range(5):
            # TODO Update the line below to take into account the piece rotation
            if(available_tetris_pieces()[piece['shape']][0][row][col] != '.'):
                matrix[piece['row']+row][piece['column']+col] = 'c'
    return matrix

def isValidPosition(game_matrix, piece, adjColumn=0, adjRow=0):
    # TODO Update the code to take into account the piece rotation
